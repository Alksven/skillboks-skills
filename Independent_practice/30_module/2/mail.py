old_list = [{'name': 'Kenneth', 'score': 3}, {'name': 'Bebe', 'score': 41},
...
new_list = sorted(old_list, key=lambda x: x['score'])
min_list = min(old_list, key=lambda x: x['score'])
max_list = max(old_list, key=lambda x: x['score'])
print(min_list)
print(max_list)
